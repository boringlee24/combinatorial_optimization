...
for ins in instances:
    ...
    x = np.array(x_data['lp'][index])
    y = (np.array(y_data['lp'][index]) - np.array(y_data['ilp'][index])) / np.array(y_data['lp'][index]) * 100
    marker = 'd'
    color = 'deepskyblue'
    ALGO = 'LP ILP Gap'
    axs[index].plot(x, y, label=ALGO, marker=marker, color=color)
    if index < 2:
        axs[index].yaxis.set_major_locator(MultipleLocator(10))
#        axs[index].set_ylim(0,1.5)
    else:
        axs[index].yaxis.set_major_locator(MultipleLocator(20))
#        axs[index].set_ylim(0,3)
...
axs[0].set_ylabel('LP Bound and ILP\nExact Solution Gap (%)', fontsize=14)

plt.savefig('plots/lp_vs_ilp.pdf')
