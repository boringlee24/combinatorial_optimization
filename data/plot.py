...
algos = ['bruteforce', 'greedy']
ALGOS = ['EXHAUSTIVE', 'GREEDY']
...
for ins in instances:
    ...
    for algo in algos:
        ...
        y = np.array(y_data[algo][index]) / max(y_data['bruteforce'][index])
        if algo == 'bruteforce':
            marker = 'd'
            color = 'deepskyblue'
        else:
        ...
        if index < 3:
            ...
            axs[index].yaxis.set_major_locator(MultipleLocator(0.5))
...
axs[0].set_ylabel('Optimal Sum\n(Norm. Exhausive Max)', fontsize=14)

plt.savefig('plots/greedy.pdf')
