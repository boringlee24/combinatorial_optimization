...
algos = ['tabu_1min', 'tabu_10min', 'bruteforce']
ALGOS = ['TABU(1min)', 'TABU(10min)', 'EXHAUSTIVE']
...
for ins in instances:
    ...
    for algo in algos:
        ...
        y = np.array(y_data[algo][index]) / max(y_data['tabu_1min'][index])
        if algo == 'bruteforce':
            marker = 'd'
            color = 'deepskyblue'
        elif algo == 'tabu_1min':
...
axs[0].set_ylabel('Optimal Sum\n(Norm. TABU (1min) Max)', fontsize=14)

plt.savefig('plots/tabu.pdf')
